    ...
    def execute(self, graph_info: dict, runner: QAOARunner, console) -> None:
        ...
        if exact_maxcut_val == -1:
            try:
                ...
                exact_maxcut_val = find_exact_maxcut(graph)['max_cut_value']
            except Exception:
                pass

        if exact_maxcut_val != -1 and exact_maxcut_val is not None and exact_maxcut_val > 0:
            ratio = len(cut_edges) / exact_maxcut_val
            ax1.set_title(f"1. Grafo e Taglio Massimo\nMigliore Partizione: {best_bitstring} (Taglio: {len(cut_edges)}/{exact_maxcut_val}, Ratio: {ratio:.4f}, Costo Atteso: {-trajectory_cuts[-1]:.4f}, Costo Migliore: {-len(cut_edges)})\n"
                          f"Parametri Ottimi: $\\gamma$ = {gammas[-1]:.4f}, $\\beta$ = {betas[-1]:.4f}", fontsize=13, fontweight='bold', pad=15)
        else:
            ax1.set_title(f"1. Grafo e Taglio Massimo\nMigliore Partizione: {best_bitstring} (Rami tagliati: {len(cut_edges)}, Costo Atteso: {-trajectory_cuts[-1]:.4f}, Costo Migliore: {-len(cut_edges)})\n"
                          f"Parametri Ottimi: $\\gamma$ = {gammas[-1]:.4f}, $\\beta$ = {betas[-1]:.4f}", fontsize=13, fontweight='bold', pad=15)
        ax1.axis('off')
